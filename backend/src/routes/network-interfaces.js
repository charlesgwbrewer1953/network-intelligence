...
router.post('/', async (req, res) => {
  const { name, display_name, interface_type, ip_address, subnet_mask, cidr, mac_address, gateway_ip, network_id, ssid } = req.body;
  if (!name) return res.status(400).json({ error: 'name is required' });

  const { rows } = await pool.query(
    `INSERT INTO network_interfaces
       (name, display_name, interface_type, ip_address, subnet_mask, cidr, mac_address, gateway_ip, network_id, ssid)
    ...
    [name, display_name || null, interface_type || null, ip_address || null,
     subnet_mask || null, cidr || null, mac_address || null, gateway_ip || null, network_id || null, ssid || null]
  );
